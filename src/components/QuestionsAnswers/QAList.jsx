...
const QAList = ({ message }) => {
  ...
  const mapSearch = mapQuestions.map((el, index) => {
    ...
    while (index < QCount && key.length >= 1) {
      ...
      return (
        <>
          <div key={el} className='QA p-4 border-b border-gray-400 dark:border-gray-400' data-testid="Questions">
  ...
  if (data.length === 0) {
    ...
    return (
      <>
        <div data-testid="scroll" role="region" className='p-8 border dark:border-gray-700 overflow-y-auto h-128 ...'>
          {mapSearch}
          <div className='flex space-x-2'>
            <label>
